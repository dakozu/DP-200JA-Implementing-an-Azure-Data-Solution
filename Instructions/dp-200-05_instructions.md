...
1. [新規] ブレードで、[**マーケット プレースの検索**] テキスト ボックスに移動し、「**Synapse**」という単語を入力します。表示される一覧の [**Azure Synapse Analytics (以前の SQL DW)**] をクリックします。

1. [**Azure Synapse Analytics (以前の SQL DW)**] ブレードで [**作成**] をクリックします。

1. **SQL Data Warehouse** ブレードから、次の設定を使用して Azure Synapse Analytics を作成します。
    ...
    - データ ソースの **追加設定** タブで、**サンプル** をクリックする

    - これが完了したら、[**基本**] タブをクリックします。
    
    - [データベースの詳細] セクションで、次の情報を入力する

        - **データベース ウェアハウス名**: **Warehousexx** (**xx** は自分のイニシャル)

        - **サーバー**: 次の設定で [**新規作成**] をクリックして新しいサーバーを作成し、[**OK**] をクリックします。
            - **サーバー名**: **dwhservicexx** (**xx** は自分のイニシャル)
            - **サーバー管理者のログイン**: **xxsqladmin** (**xx** は自分のイニシャル)
            - **パスワード**: **Pa55w.rd**
            - **パスワードを確認**: **Pa55w.rd**
            - **場所**: 近くの **場所** を選択します。
            - [Azure サービスがサーバにアクセスすることを許可する] チェックボックスをオンにします。
            - [**OK**] をクリックします。

                ![Azure portal でサーバー インスタンスを作成する](Linked_Image_Files/M05-E02-T01-img01.png)

    - パフォーマンス レベル：[**パフォーマンス レベルの選択**] をクリックし、[**Gen2 DW100C**] を選択します。

        ![Azure portal での Azure Synapse Analytics のパフォーマンスの構成](Linked_Image_Files/M05-E02-T01-img02.png)

    - [**適用**] をクリックすると、次の構成が表示されます。

        ![Azure portal での Azure Synapse Analytics を構成する](Linked_Image_Files/M05-E02-T01-img03.png)

1. [**SQL Data Warehouse**] 画面で、[**作成**] をクリックします。
...
1. [**dwhservicexx**] 画面で、[**ファイアウォールと仮想ネットワーク**] をクリックします。

1. [dwhservicexx - ファイアウォールと仮想ネットワーク] 画面で、オプション [**+ クライアント IP の追加**] をクリックし、[**保存**] をクリックします。成功画面で、[**OK**] をクリックします。
