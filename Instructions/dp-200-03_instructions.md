...
1. **クラスターの作成** スクリーンの 「新規クラスター」 で、次の設定を使って Databricks クラスターを作成し、「**クラスターの作成**」 をクリックします。
    ...
    - **Databricks ランタイムのバージョン**: **ランタイム：6.6 (Scala 2.11、Spark 2.4.5)**
...
- **08-Parsing-Nested-Data** -  このノートブックは、オプション サブフォルダーにあり、後で都合の良い時に試してみるためのサンプル プロジェクトが含まれています。
...
- **08-Custom-Transformations** - このノートブックは、オプションのサブフォルダーにあり、後で自分の時間で探索する為のサンプルプロジェクトが含まれている。
