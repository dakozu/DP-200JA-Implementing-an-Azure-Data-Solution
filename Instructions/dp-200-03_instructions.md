...
1. アプリケーションの **DLAccess** の **名前** と **https://adventureworks.org/exampleapp** の **URL** を入力します。        作成するアプリケーションの種類に応じて、[**Webアプリ/ API**] を選択します。値を設定したら、[**作成**] を選択します。DLAccess 画面が表示されます。 

1. [**DLAccess** 登録アプリ] 画面で、[**アプリケーションID**] をコピーしてして メモ帳 に貼り付けます。   

1. [**DLAccess** 登録アプリ] 画面で、[**設定**] ボタンをクリックし、[**キー**] オプションをクリックします。

1. **DL アクセス キー** の **説明** を入力し、キーの **期間** を **1 年間** に指定します。完了したら、[**保存**] をクリックします。 

    >**重要**: [保存] をクリックすると、キーが表示されます。このキー値をコピーできるのは 1 度だけです。
