...
1. **awcdbstudxx** 画面で、**+ コンテナーの追加** をクリックします。これにより、**SQL API** ブレードを使用して、**awcdbstudxx データ エクスプローラー** ブレードを開きます。   

1. **SQL API** ブレードで、**新しいコンテナー** 画面をクリックし、次の設定で Clothing というコンテナーを使用して Products データベースを作成します。
...
1. **SQL API** ペインで、**衣料品** を展開し、**アイテム** をクリックします。新しいドキュメントが表示され、置き換えるサンプル JSON が表示されます。
...
1. [ドキュメント] ウィンドウで、[**新規アイテム**] (新規ドキュメント) のアイコンをクリックします。
...
1. ID が 1 である最初のアイテムの値は **33218896**、2番目のアイテムは **33218897** となります 
...
1. Azure portal の **アイテム** 画面で、ドロップダウン ボタンの **新規ストアド プロシージャ** をクリックし、**新しい UDF** をクリックします。
...
## エクササイズ3：Azure Cosmos DB を使用してデータをグローバルに分散する
...
> **注記**：これまでにラボを完了し、時間がある場合、演習 4 を実行できるかどうかをインストラクターに確認してください。これは、Cosmos DB に対してアプリケーションを構築する例です。これは DP200 の試験要件ではなく、このラボでは何が可能かを示します
