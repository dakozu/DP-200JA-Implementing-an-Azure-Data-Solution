...
5. 「**API オプションの選択**] 画面で **コア(SQL)** の作成をボタンを押して
   「**Azure Cosmos DB アカウントの作成**」 画面から、次の設定を使用して Azure Cosmos DB アカウントを作成します。
...
4. 「**コンテナーの追加**」 ブレードで、**New Container** をクリックして、以下の設定で「Clothing」という名前のコンテナーを含む製品データベースを作成します。
...
1. 「**awcdbstudcto - データ エクスプローラー**」 画面で、データ エクスプローラー ツールバーの 右側で、「**Open Full Screen**」 ボタンをクリックします。「全画面表示」 ダイアログ ボックスで、　**Read and Write** の「**Open**」 をクリックします。Microsoft Edge で新しいタブが開きます。

2. 「**SQLAPI**」 ペインで、更新アイコンをクリックし、「**Products**」 を展開して 「**Clothing**」 をクリックし、「**Items**」 をクリックします。 

3. ツールバーの、「**New Item**」 のアイコンをクリックします。新しいドキュメントが表示され、置き換えるサンプル JSON が表示されます。

4. 次のコードをコピーし、**Items** タブに貼り付けます。
...
5. 　JSON を追加したら、「**Save**」 をクリックします。

6. ツールバーの「**New Item**」 のアイコンをクリックします。

7. 次のコードをコピーし、「**Items**」 タブに貼り付けます。
...
1. 開いた Microsoft Edgeブラウザーで、データ エクスプローラーの ツールバーにある 「**New SQL Query**」 ボタンをクリックします。
...
1. 開いた Microsoft Edge ブラウザーで、データ エクスプローラーのツールバーにある 「**New Stored Procedure**」 ボタンをクリックします。これは、Open Queryのボタンの横にあります。

    > **注**: サンプルのストアド プロシージャを示す新しいストアド プロシージャ画面が表示されます。

2. 「新規ストアド プロシージャ」 画面で、「**Stored Procedure Id**」 テキスト ボックスに「**createMyDocument**」と入力します。
...
4. 「New Stored Procedure」 画面で、「**Save**」 をクリックします。

5. 「New Stored Procedure」 画面で、「**Execute**」 をクリックします。

6. 「Input Parameters」 画面で、「**Partition key value**」 を設定し、「**Type**」 を 「**String**」 に、「**Value**」 を「**33218898**」に設定し、他の設定はそのままにして、「**Execute**」 をクリックします。
...
7. 開いた Microsoft Edge ブラウザーで、データ エクスプローラーの 「**New Stored Procedure**」 のドロップ ダウン ボタンをクリックして 「**New UDF**」 をクリックします。

    > **注**: **function userDefinedFunction(){}** を示す　New UDF1 タブが表示されます。

8. 「New UDF1」 タブで、「**User Defined Function Id**」 テキスト ボックスに「**producttax**」と入力します。
