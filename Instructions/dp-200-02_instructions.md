...
6. 「**ストレージ アカウントの作成**」 画面で、「**レビューと作成**」 をクリックします。
...
1. Azure portal の 「**awsastudxx - Containers**」 画面で、リストから 「**画像**」 項目をクリックします。

2. 「**画像**」 画面で、「**アップロード**」 ボタンをクリックします。
...
7. 「**ストレージ アカウントの作成**」 ブレードで、「**レビューと作成**」 をクリックします。
...
15. 手順を繰り返して、preferences.JSON ファイルを **Labfiles\Starter\DP-200.2\logs** フォルダーから Data Lake Store gen2 の**データ** ファイル システムにアップロードします。
   ...
   > **注**: ファイルのアップロードの所要時間は約 5 秒です。「**ビューが最新でない可能性があります。更新しますか?**」というメッセージが Azure Storage Explorer に表示されます。 **「はい」** をクリックします。完了すると、2 つのファイルすべてが BLOB のアップロード画面のリストに表示されます。
